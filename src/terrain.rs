...
impl Terrain {
    ...
    pub(crate) fn generate_chunk_vertices(
        chunk: Arc<RwLock<Box<Chunk>>>,
        x_pos_chunk: Option<&Arc<RwLock<Box<Chunk>>>>,
        x_neg_chunk: Option<&Arc<RwLock<Box<Chunk>>>>,
        z_pos_chunk: Option<&Arc<RwLock<Box<Chunk>>>>,
        z_neg_chunk: Option<&Arc<RwLock<Box<Chunk>>>>,
    ) -> Option<Vec<Vertex3D>> {
    ...
    pub fn update_meshes(&mut self, gl_resources: &mut GLResources) {
        ...
        for (index, chunk) in self.chunks.iter_mut() {
            ...
            match chunk.next_update {
                ChunkUpdate::Generated | ChunkUpdate::BlockUpdate(_) | ChunkUpdate::NeighborChanged(_) => {
                    println!("pushed {:?} to the queue for mesh regen", index);
                    self.mesh_rebuild_queue.push(*index);
                },
                ChunkUpdate::NoUpdate => continue,
            }
        }
        ...
        while let Some(index) = self.mesh_rebuild_queue.pop() {
            if let Some(chunk) = self.chunks.get_mut(&index) {
                ...
                println!("rebuilding mesh for {:?}", index);
                {
                    let chunk = chunk.write().unwrap();
                    match chunk.next_update {
                        ChunkUpdate::NeighborChanged(_) | ChunkUpdate::NoUpdate => {},
                        _ => {
                            let adjacent_chunks = [
                                index + ChunkIndex::new(1, 0),  //x_pos
                                index + ChunkIndex::new(-1, 0), //x_neg
                                index + ChunkIndex::new(0, 1),  //z_pos
        ...
        /* let rebuild_queue = self.mesh_rebuild_queue.clone();
        for index in rebuild_queue {
            if let Some(chunk) = self.chunks.at_index(&index) {
                self.update_single_chunk_mesh(&index, gl_resources);
            }
        } */

        /* for _ in 0..10 {
            if let Some(index) = self.mesh_rebuild_queue.pop() {
                if let Some(_) = self.chunks.at_index_mut(&index) {
                    self.update_single_chunk_mesh(&index, gl_resources);
                }
                else {
                    self.mesh_rebuild_queue.push(index);
